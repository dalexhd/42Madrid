...
/*   Updated: 2020/01/27 15:44:10 by aborboll         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			apply_flags(t_info *info, int isneg)
{
	if (!isneg)
	{
		if (info->flags[F_Space] == 1 &&
		info->flags[F_Plus] != 1 && info->type != 'u')
			info->res = ft_strjoin_free(ft_strdup(" "), info->res);
	}
	else
		info->res = ft_strjoin_free(ft_strdup("-"), info->res);
	if (info->flags[F_Hash] == 1
		&& ft_strcmp(info->res, "0") && info->type != 'o')
		info->res = ft_strjoin_free(
			ft_strdup(info->type == 'X' ? "0X" : "0x"), info->res);
	else if (info->flags[F_Hash] == 1 &&
		ft_strcmp(info->res, "0") != 0 && info->type == 'o')
		info->res = ft_strjoin_free(ft_strdup("0"), info->res);
	if (info->flags[F_Plus] == 1 && info->res[0] != '-' && info->type != 'u')
		info->res = ft_strjoin_free(ft_strdup("+"), info->res);
}
...
void			do_number(t_info *info)
{
	...
	if (info->flags[F_Zero] != 1 ||
	info->flags[F_Neg] == 1 || info->precision > 0)
